...
Модули — генерируемые предметы с бафами и дебафами.
5 редкостей: от обычной (⚪) до легендарной (🟡).

Основные возможности:
- Генерация модулей со случайными эффектами
- Улучшение до следующей редкости
- Разборка на материалы
- Установка в слоты (1-3)
"""
import random
import json
...
class Rarity(IntEnum):
    ...
    Attributes:
        COMMON (1): ⚪ Обычная — 1-2 бафа, 2-3 дебафа (70%)
        RARE (2): 🟢 Редкая — 2 бафа, 1-2 дебафа (20%)
        EPIC (3): 🟣 Эпическая — 2-3 бафа, 1-2 дебафа (7%)
        MYTHIC (4): 🔴 Мифическая — 3-4 бафа, 0-1 дебаф (2.5%)
        LEGENDARY (5): 🟡 Легендарная — 4 бафа, 0-1 дебаф (0.5%)
    """
    COMMON = 1      # ⚪ Обычная
    RARE = 2        # 🟢 Редкая
    EPIC = 3        # 🟣 Эпическая
...
class ModuleSystem:
    ...
    Отвечает за:
    - Генерацию модулей со случайными эффектами
    - Улучшение модулей до следующей редкости
    - Расчёт наград за разборку
    - Форматирование карточек для отображения
    ...
    Example:
        ...
        >>> card = ModuleSystem.format_module_card(module)
        >>> print(card)
        AB-12 #1
        ...
    """
    ...
    def roll_rarity() -> Rarity:
        """
        Определить редкость модуля случайным образом.
        
        Returns:
            Rarity: Выпавшая редкость согласно RARITY_CHANCES
        """
        roll = random.random()
        cumulative = 0.0
