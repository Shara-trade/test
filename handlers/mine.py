...
async def on_mine_click(callback: CallbackQuery):
    try:
        ...
        # Бонусы от модулей (новая система)
        module_bonuses = await db.get_active_module_bonuses(user_id)
        
        # Применяем бонусы модулей
        # Баф: asteroid_resources увеличивает добычу
        mining_bonus_from_modules = module_bonuses.get('asteroid_resources', 0) / 100  # Проценты
        # Баф: container_chance увеличивает шанс лута
        loot_bonus_from_modules = module_bonuses.get('container_chance', 0) / 100
        # Баф: rare_asteroid_chance увеличивает шанс редкого астероида
        rare_asteroid_bonus = module_bonuses.get('rare_asteroid_chance', 0) / 100
        # Баф: max_energy добавляет к максимальной энергии
        max_energy_bonus = module_bonuses.get('max_energy', 0)
        # Баф: heat_reduction снижает нагрев за клик
        heat_reduction_bonus = module_bonuses.get('heat_reduction', 0)
        
        # === КРИТ-СИСТЕМА ===
        crit_chance = 0.03  # 3% базовый шанс крита
        ...
        if loot_item:
            ...
            await db.add_item(user_id, loot_item.key, 1)
        ...
        if container_info:
            # Контейнеры хранятся в инвентаре как предметы
            # Проверяем лимит (максимум 10 каждого типа)
            container_item_key = f"container_{container_info.container_type.value}"
            existing = await db.get_user_item(user_id, container_item_key)
            current_count = existing.get('quantity', 0) if existing else 0
            
            if current_count < 10:  # Максимум 10 контейнеров каждого типа
                container_text = f"\n{container_system.format_container_drop(container_info)}"
                # Добавляем контейнер в инвентарь
