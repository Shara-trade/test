...
CONTAINER_TYPES = ["common", "rare", "epic", "mythic", "legendary", "ksm"]
...
@router.message(F.text.regexp(r'(?i)^(открыть\s+.+?\s+контейнер|отк\s+.+|open\s+.+?\s+container)$'))
...
async def open_container_by_type(message: Message, user_id: int, container_type: str):
    ...
    if container_type == 'ksm':
        module = rewards.get('module')
        if module:
            # Сохраняем модуль в БД
            result = await db.create_module(
                user_id,
                module['name'],
                module['rarity'],
                module['buffs'],
            ...
            if result.get('success'):
                from game.modules import RARITY_EMOJI, RARITY_NAME, Rarity
                
                rarity = Rarity(module['rarity'])
                rarity_emoji = RARITY_EMOJI[rarity]
                module_id = result['module_id']
                
                text = f'🎉<b>Поздравляем! Из контейнера выпал:</b>\n\n'
                text += f'{rarity_emoji} {module["name"]} #{module_id}\n\n'
                
                text += '💚 <b>Бафы:</b>\n'
                for key, value in module['buffs'].items():
                    from game.modules import BUFF_NAMES
                    name = BUFF_NAMES.get(key, key)
                    text += f'• {name}: +{value}%\n'
                
                text += '\n❤️ <b>Дебафы:</b>\n'
                for key, value in module['debuffs'].items():
                    from game.modules import DEBUFF_NAMES
                    name = DEBUFF_NAMES.get(key, key)
                    text += f'• {name}: +{value}%\n'
                
                text += '\n<i>Модуль добавлен в инвентарь</i>'
