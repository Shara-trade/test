    ...
    for i, module in enumerate(available_modules[:8]):  # Максимум 8 кнопок
        ...
        keyboard_buttons.append([
            InlineKeyboardButton(
                text=f'{icon} {name}',
                callback_data=f'module_select_{item_key}'
            )
        ])
...
@router.callback_query(F.data.startswith('module_select_'))
async def on_module_select(callback: CallbackQuery):
    """Выбор места установки модуля"""
    item_key = callback.data.replace('module_select_', '')
    user_id = callback.from_user.id
    ...
    if player_slots["available"] > 0:
        keyboard_buttons.append([
            InlineKeyboardButton(
                ...
                callback_data=f'module_install_{item_key}_player'
            )
        ])
        ...
        if installed < max_slots:
            ...
            keyboard_buttons.append([
                InlineKeyboardButton(
                    ...
                    callback_data=f'module_install_{item_key}_drone_{drone_id}'
                )
            ])
...
@router.callback_query(F.data.startswith('module_install_'))
async def on_module_install(callback: CallbackQuery):
    """Установка модуля"""
    parts = callback.data.split('_')
    item_key = parts[2]
    target = parts[3]
    drone_id = parts[4] if len(parts) > 4 else None
...
async def on_uninstall_menu(callback: CallbackQuery):
    ...
    for module in installed:
        ...
        keyboard_buttons.append([
            InlineKeyboardButton(
                text=f'{icon} {name} - {location}',
                callback_data=f'module_uninstall_{item_key}'
            )
        ])
...
@router.callback_query(F.data.startswith('module_uninstall_'))
async def on_module_uninstall(callback: CallbackQuery):
    """Снятие модуля"""
    item_key = callback.data.replace('module_uninstall_', '')
    user_id = callback.from_user.id
